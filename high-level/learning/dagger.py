...
# DAgger 的默认配置。这个类继承自 skrl.Agent，所以保留了一些 PPO/skrl 常见字段；
# 当前文件真正用于 DAgger 蒸馏的核心字段是 rollouts、learning_epochs、mini_batches、learning_rate、
# grad_norm_clip、entropy_loss_scale、learning_starts，以及 fixed_base/reach_only/pred_success 等运行时附加字段。
DAGGER_DEFAULT_CONFIG = {
    "rollouts": 16,                 # 每收集多少个环境 step 后更新一次 student；训练入口会覆盖为 24。
    "learning_epochs": 6,           # 每次更新时，把 memory 中的数据重复训练多少轮。
    ...
    "learning_rate_scheduler": None,        # 可选学习率调度器；训练入口将其设为 None。
    "learning_rate_scheduler_kwargs": {},   # 学习率调度器参数；只有 learning_rate_scheduler 非 None 时才会使用。

    "state_preprocessor": None,             # 可选观测预处理器；当前 DAgger 更新中相关调用被注释掉。
...
class DAgger(Agent):
    # 非 RNN 版本的 DAgger agent。
    # 它不训练 teacher，也不使用 RL reward 优化策略；它只把 student policy 的输出拟合到 teacher_actions。
    def __init__(self,
        ...
        self.state_space = state_space  # student_obs 的空间；命名为 state_space 是因为 wrapper 用 states["states"] 存 student 观测。
        
        # models
        self.policy = self.models.get("policy", None)  # 需要训练的 student policy；它接收视觉观测并输出动作。
        ...
        self._time_limit_bootstrap = self.cfg["time_limit_bootstrap"]  # 兼容字段；当前 DAgger loss 不使用。
        
        self._fixed_base = self.cfg.get("fixed_base", False)  # 固定底盘训练时，loss 忽略最后 2 个底盘动作维度。
        self._reach_only = self.cfg.get("reach_only", False)  # 只训练 reach 相关动作时，loss 采用特定动作维度组合。
        self._pred_success = self.cfg.get("pred_success", False)  # 额外预测是否已经 lifted，用一个附加输出和 lifted_now 监督。
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(self.action_space, int):
            stu_action_space = self.action_space  # action_space 直接是整数时，它就是动作维度。
        else:
            stu_action_space = self.action_space.shape[0]  # gym Space 时，从 shape[0] 读取动作维度。
        if self._pred_success:
            stu_action_space += 1  # pred_success 会让 student 多输出 1 维，用来预测 lifted_now。
        
        if self.memory is not None:
            self.memory.create_tensor(name="student_obs", size=self.state_space, dtype=torch.float32)  # student 输入；DAgger 更新时会喂给 student policy。
            ...
            self.memory.create_tensor(name="actions", size=stu_action_space, dtype=torch.float32)  # student 当时输出的动作；可用于调试或替代 loss。
            ...
            self.memory.create_tensor(name="terminated", size=1, dtype=torch.bool)  # episode 结束标记；memory 和日志需要。
            self.memory.create_tensor(name="log_prob", size=1, dtype=torch.float32)  # 策略 log_prob；当前 deterministic policy 时可能为 0。
            self.memory.create_tensor(name="lifted_now", size=1, dtype=torch.float32)  # pred_success 分支的监督标签。
            
            # tensors sampled during training
            self._tensor_names = ["student_obs", "teacher_obs", "actions", "teacher_actions", "rewards", "terminated", "log_prob", "lifted_now"]  # _update 会按这个顺序从 memory 采样。
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        if timestep < self._random_timesteps:
            return self.policy.random_act({"states": states}, role="policy")  # 可选随机探索；本项目配置通常不会进入。
        
        # sample stochastic actions
        actions, log_prob, outputs = self.policy.act({"states": states}, role="policy")  # 用 student policy 根据 student_obs 输出动作。
        if log_prob is None:
    ...
    def record_transition(self, student_obs: torch.Tensor, teacher_obs, actions: torch.Tensor, teacher_actions: torch.Tensor, rewards: torch.Tensor, next_states: torch.Tensor, terminated: torch.Tensor, truncated: torch.Tensor, infos: Any, timestep: int, timesteps: int) -> None:
        ...
        if self.memory is not None:
            ...
            lifted_now = infos.get("lifted_now", torch.zeros_like(terminated, dtype=torch.float32, device=terminated.device))  # 没有 lifted_now 时用 0 标签占位。
            self.memory.add_samples(student_obs=student_obs, teacher_obs=teacher_obs, actions=actions, teacher_actions=teacher_actions, rewards=rewards,
                                    terminated=terminated, log_prob=self._current_log_prob, lifted_now=lifted_now)  # 把 DAgger 训练需要的 student_obs 和 teacher_actions 聚合到数据集。
    ...
    def post_interaction(self, timestep: int, timesteps: int) -> None:
        ...
        if not self._rollout % self._rollouts and timestep >= self._learning_starts:
            self.set_mode("train")  # 切到训练模式，启用 PyTorch 模块里的 training 行为。
            self._update(timestep, timesteps)  # 用 memory 中聚合的数据训练 student 拟合 teacher。
            self.set_mode("eval")  # 更新后切回 eval，继续稳定采样动作。
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        :type timesteps: int
        """
        cumulative_dagger_loss = 0.0  # 累计本次更新内所有 mini-batch 的 imitation loss。
        cumulative_entropy_loss = 0.0  # 累计可选 entropy loss；默认权重为 0。
        
        # learning epochs
        for epoch in range(self._learning_epochs):  # 多次遍历同一批聚合数据，提高样本利用率。
            sampled_batches = self.memory.sample_all(names=self._tensor_names, mini_batches=self._mini_batches)  # 从 memory 按 _tensor_names 取出全部数据并切成 mini-batch。
            kl_divergences = []  # 旧 KL 早停逻辑的容器；当前 KL 计算代码已注释。
            for sampled_student_obs, sampled_teacher_obs, sampled_actions, sampled_teacher_actions, sampled_rewards, sampled_terminated, sampled_log_prob, sampled_lifted_now in sampled_batches:
                ...
                if self._entropy_loss_scale:
                    ...
                    entropy_loss = 0.0  # 默认没有熵正则，只有 DAgger MSE。
                    
                # compute policy loss
                # dagger_loss = torch.norm(sampled_actions - sampled_teacher_actions, dim=-1).mean()
                student_actions, _, _ = self.policy.act({"states": sampled_student_obs}, role="policy")  # 用采样到的 student_obs 重新前向计算 student 当前动作。
                if self._pred_success:
                    student_actions, pred_lifted = student_actions[:, :-1], student_actions[:, -1]  # 最后一维单独作为 lifted_now 预测，不参与动作 MSE。
                if self._fixed_base:
                ...
                if self._pred_success:
                    dagger_loss += F.mse_loss(pred_lifted.squeeze(), sampled_lifted_now.squeeze())  # 额外把 lifted_now 预测误差加到总 loss。
            ...
            if self._learning_rate_scheduler:
                if isinstance(self.scheduler, KLAdaptiveLR):
                    self.scheduler.step(torch.tensor(kl_divergences).mean())  # KLAdaptiveLR 需要 KL 均值作为调度信号；当前通常不会启用。
                else:
                    self.scheduler.step()  # 普通 PyTorch scheduler 每个 epoch 调一次。
